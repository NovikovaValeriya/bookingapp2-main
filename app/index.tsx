...
import { View, Text, Image, TouchableOpacity, StyleSheet, SafeAreaView, StatusBar,  Modal, FlatList, Dimensions } from "react-native";
...
export default function App() {
    ...
    const markedDates = useMemo(() => {
        const dates: { [key: string]: { selected: boolean; marked?: boolean; selectedColor?: string; color?: string; customStyles?: any  } } = {};
        ...
        <SafeAreaView style={styles.safeArea}>
            ...
            <View style={styles.containerAboveDetails}>
                <View style={styles.details}>
                    ...
                    {datesSelected && (  
                        <TouchableOpacity style={styles.priceSection1} onPress={() => router.push ('/booking')}>
...
const styles = StyleSheet.create({
    ...
    separatorImage: {
        width: width * 0.45, // яизменил на это для того чтобы на разных экранах не ломалась картинка 
        height: 60,
        resizeMode: 'contain',
    },
    dateContainer: {
    ...
    discountImage: { //вообще ничё не меняется почему?!?!?!?!??!?!???!?!?!?!??!????????????
        width: 400,
        height: 100,
        borderRadius: 12,
